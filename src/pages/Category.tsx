...
// Load More button implementation (replaced infinite scroll)
...
const Category: React.FC = () => {
    ...
    // Simple state for progressive display
    const [displayCount, setDisplayCount] = useState(20); // How many products to show
    
    // Get ALL data using the existing hook
    const {
    ...
    }, [sort, price, brands, minRating, inStockOnly, selectedRam, selectedStorage, selectedNetwork]);

    // Load More functionality - show 20 more from all data
    const handleLoadMore = useCallback(() => {
        setDisplayCount(prev => prev + 20);
    }, []);
    
    // Convert ALL products to CategoryProduct format
    const allCategoryProducts: CategoryProduct[] = useMemo(() => {
        if (!allVariantsData) return [];
        
        const fallbackImages = [imgHeadphones, imgSneakers, imgWatch, imgLaptop, imgController, imgCoffee];
        
        return allVariantsData.map((variant, index) => {
    ...
    const filterOptions = useMemo(() => {
        ...
        allCategoryProducts.forEach(product => {
            ...
            if (product.brand && product.brand !== 'Various') {
                ...
                // Extract brand ID from original variant data
                if (product.originalVariant?.brand?.id) {
                    brandIdMap.set(product.brand, product.originalVariant.brand.id);
                }
            }
        });
    ...
    // Check if any actual filters are applied (not just sorting)
    const hasActiveFilters = useMemo(() => {
        return price[0] > 0 || price[1] < 200000 || minRating > 0 || 
               brands.length > 0 || inStockOnly || selectedRam || 
               selectedStorage || selectedNetwork.length > 0;
    }, [price, minRating, brands, inStockOnly, selectedRam, selectedStorage, selectedNetwork]);

    // Filter ALL products - no limits, get all filtered results
    const allFilteredProducts = useMemo(() => {
        let list = [...allCategoryProducts];

        // Apply price filters
        if (price[0] > 0) {
            list = list.filter((p) => p.lowestPrice >= price[0]);
        }
        if (price[1] < 200000) {
            list = list.filter((p) => p.lowestPrice <= price[1]);
        }
        ...
        return list; // Return ALL filtered results, no limits
    }, [allCategoryProducts, price, minRating, brands, inStockOnly, selectedRam, selectedStorage, selectedNetwork]);

    // Show only the first displayCount products from filtered results
    const displayedProducts = useMemo(() => {
        return allFilteredProducts.slice(0, displayCount);
    }, [allFilteredProducts, displayCount]);

    // Check if there are more filtered products to show
    ...
    if (isLoading && allFilteredProducts.length === 0) {
        return (
                ...
                <div className="container py-6">
                    ...
                    <p className="text-muted-foreground mt-2">
                        {isLoading && allFilteredProducts.length === 0
                            ? "Loading products..."
                            : hasActiveFilters
                                ? `Showing ${allFilteredProducts.length} filtered products`
                                : `${totalVariantsCount || 0} products available`
                        }
                    </p>
                </div>
            </header>
            ...
            <main className="container py-6 md:py-8">
                ...
                <div className="mb-6 flex items-center justify-between gap-3">
                    <div className="text-sm text-muted-foreground">
                        {hasActiveFilters 
                            ? `Showing ${displayedProducts.length} products out of ${allFilteredProducts.length} products`
                            : `Showing ${displayedProducts.length} products (${totalVariantsCount || 0} available)`
                        }
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <section className="flex-1 w-full">
                        ...
                        {!hasMoreProducts && displayedProducts.length > 0 && (
                            <div className="mt-8 text-center text-muted-foreground space-y-2">
                                <p className="text-sm">✨ You've seen all {allFilteredProducts.length} products</p>
                                <Button 
                                    variant="ghost" 
                                    size="sm" 
                        ...
                        {/* Error message */}
                        {error && (
                            <div className="mt-8 text-center text-destructive space-y-2">
